...
(*Print[Coefficient[LinearTerm,QuantumField[Field[[i]]]]+TP$Sigma[QuantumField[Field[[i]]]]==0];*)
...
Msq=Range[nField];
For[k=1,k<=nField,k++,
Msq[[k]]=-(PP[0][[k,k]])/.((#->0)&/@CT)/.{PP$Sigma[arg1_,arg2_][arg3_]:>0,dTh1->0,dTh2->0};
];
Print[Msq];
...
ScalarSingleRen[Vertex_List,FieldIn_List,CT_List]:=Block[{Field,nField,PP,Msq,EquationsForFieldNormalization,EquationsForMassNormalization,RenSolution,k},
nField=Length[FieldIn];
(*Handle the input Field to a good format*)
Field=If[nField!=2,{FieldIn[[1]],FieldIn[[1]]},FieldIn];
Print[Field];
(*Get the inverse Propagator matrix*)
PP[p2_]:=Cases[Vertex,
FRVertex[{f:{QuantumField[Field[[1]]],QuantumField[Field[[2]]]}|{QuantumField[Field[[2]]],QuantumField[Field[[1]]]},lo_,nlo_}]
:>((lo+nlo+I (PP$Sigma@@f)[p2])/I/.{Pair[_Momentum,_Momentum]:>p2}//Simplify)
][[1]];
Print[PP[p2]];
(*Get the mass involved*)
Msq=-(PP[0])/.((#->0)&/@CT)/.{PP$Sigma[arg1_,arg2_][arg3_]:>0,dTh1->0,dTh2->0};
Print[Msq];
(*Get the solution for the renormalization constant*)
EquationsForFieldNormalization={
PP'[Msq]==1(*For first field renormalization*)
};
(*We only consider physical scalar mass normalization*)
(*For the goldstone, we didn't consider gauge fixing term renormalization, so for now, just ignore it, and put its mass at zero*)
EquationsForMassNormalization={};
If[Msq=!=0,EquationsForMassNormalization={EquationsForMassNormalization,PP[Msq]==0}];
RenSolution=Solve[Join[EquationsForFieldNormalization,Flatten[EquationsForMassNormalization]],CT][[1]];
RenSolution
]
